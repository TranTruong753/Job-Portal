<script setup>
    ...
    const schema = yup.object({
        nameDegree: yup.string().required('Name of Degree or Certificate is required'),
        trainingMajor: yup.string().required('Training Major is required'),
        nameSchool: yup.string().required('Name School or Training Center is required'),
        startDate: yup
            .date()
    ...
    const { value: nameDegree, errorMessage: nameDegreeError } = useField('nameDegree');
    const { value: trainingMajor, errorMessage: trainingMajorError } = useField('trainingMajor');
    const { value: nameSchool, errorMessage: nameSchoolError } = useField('nameSchool');
    ...
    <div class="card border-0 shadow mb-4  ">
        <div class="card-body p-0">
            ...
            <div class="m-3 border border-2">
                <div class="p-3 " v-if="storeDataCv.datas[educationalInformation].length"
                    v-for="(item, index) in storeDataCv.datas[educationalInformation]" :key="index">
                    ...
                    <div class="mb-0 " @click="handleClick(index)">
                        ...
                        <div class="mx-1 mt-3 border-2 border-start pb-3 ps-3">
                            <h5 class="p-0 lh-base mb-2 text-uppercase text-primary">{{ item.nameDegree }}</h5>
                            <p class="fs-6 p-0 m-0 lh-base mb-2">
                                <b>School/Training Center</b>: {{ item.nameSchool }}
                            </p>
                            <p class="fs-6 p-0 m-0 lh-base mb-2">
                                <b>Training major</b>: {{ item.trainingMajor }}
                            </p>
                ...
                <div class="p-3 " v-else>
                    <p class="card-text">you often add educational information</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog  modal-dialog-centered  modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form @submit.prevent="onSubmit" id="formEducationalInformation">
                        <div class="row g-3 ">
                            <div class=" col-lg-12">
                                <label for="" class="mb-2">Name of Degree/Certificate<span
                                        class="text-primary">*</span></label>
                                <input v-model="nameDegree" type="text" placeholder="Name of Degree or Certificate..." class="form-control">
                                <span class="text-danger">{{ nameDegreeError }}</span>
                            </div>

                            <div class=" col-lg-6">
                                <label for="" class="mb-2">Training major<span class="text-primary">*</span></label>
                                <input v-model="trainingMajor" type="text" placeholder="Training major..." class="form-control">
                                <span class="text-danger">{{ trainingMajorError }}</span>
                            </div>

                            <div class=" col-lg-6">
                                <label for="" class="mb-2">School/Training Center<span
                                        class="text-primary">*</span></label>
                                <input v-model="nameSchool" type="text" placeholder="School/Training Center..." class="form-control">
                                <span class="text-danger">{{ nameSchoolError }}</span>
                            </div>
                            ...
                            <div class=" col-lg-6">
                                ...
                                <input v-model="endDate" type="date" placeholder="not updated" class="form-control">
                            ...
                            <div class=" col-lg-12">
                                ...
                                <textarea v-model="description" class="form-control" placeholder="Leave a Description here" id=""
                                    style="height: 22vh; resize: none;"></textarea>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" @click="deleteForm()"  data-bs-dismiss="modal">Close</button>
                    <button type="submit" class="btn btn-primary" form="formEducationalInformation">Save changes</button>
                </div>
            </div>
        </div>
    </div>
